...
def ngram_hashes(content: str, n: int, min_length: int = 5) -> Set[int]:
    ...
    [433422276]
    """
    tokens: List[str] = list(content) # NON_ALPHA.split(content.lower())
...
if __name__ == "__main__":  # pragma: no cover
    ...
    schema = StructType([
        StructField("band_idx", IntegerType(), False),
        StructField("band_hash", BinaryType(), False),
        StructField("node", LongType(), False),
    ])

    # region: MinHash
    rdd = df.select(index_column, args.column).rdd.flatMap(
    ...
    df_hash = spark.createDataFrame(rdd, schema)
    df_hash.persist(pyspark.StorageLevel.DISK_ONLY)
    df_hash.checkpoint()
    # 分组求每个 (band_idx, band_hash) 组内的最小 node
    df_min = df_hash.groupBy("band_idx", "band_hash").agg(F.min("node").alias("min_node"))

    # 与原始 DataFrame join，生成边（排除自身）
    edges_df = (
        df_hash.join(df_min, on=["band_idx", "band_hash"])
        .where(F.col("node") != F.col("min_node"))
        .select(F.col("min_node").alias("src"), F.col("node").alias("dst"))
        .distinct()
    )
    edges_df = edges_df.repartition(8096)
    df_hash.unpersist()
    edges_df.persist(pyspark.StorageLevel.DISK_ONLY)
    edges_df.checkpoint()
    ...
    with warnings.catch_warnings():
        ...
        #     df_edges.persist(pyspark.StorageLevel.DISK_ONLY)
        # )
